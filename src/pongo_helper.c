...
void *pongo_usb_callback(void *arg) {
	...
	if (checkrain_option_enabled(palerain_flags, palerain_option_setup_partial_root)) {
		LOG(LOG_NOTICE, "Please wait up to 5 minutes for the bindfs to be created.");
		LOG(LOG_NOTICE, "Once the device boots up to iOS, run again without the -B (Create BindFS) option to jailbreak.");
	} else if (checkrain_option_enabled(palerain_flags, palerain_option_setup_rootful)) {
		LOG(LOG_NOTICE, "Please wait up to 10 minutes for the fakefs to be created.");
		LOG(LOG_NOTICE, "Once the device boots up to iOS, run again without the -c (Create FakeFS) option to jailbreak.");
	}
